...
class Car:
    green_count = 0
    starting_point_rewards = 0
    three_green_zone_rewards = 0
    all_three_zones_visited = 0
    returned_after_three_zones = 0
    ...
    def __init__(self):
        ...
        self.green_zone_entries = []
        self.visited_green_zones = set()
        self.stuck_check_interval = 100
        self.stuck_distance_threshold = 50
        self.position_history = []
        self.in_green_zone = False
        self.starting_point = (100, 100)
        self.track_record = []
        self.returning_home = False
        self.visited_at_least_one_green_zone = False
        self.visited_green_zone_count = 0
    ...
    def check_starting_position_reward(self):
        if self.visited_at_least_one_green_zone:
            ...
            if distance_to_start <= starting_area_radius:
                self.is_alive = False
                if hasattr(self, 'visited_all_three'):
...
def run_car(genomes, config):
    ...
    while True:
        ...
        if remain_cars == 0 or elapsed_time >= generation_duration:
            global generation
            global unique_green_zones_count, generations_without_increase
    ...
    with open(filename, "w") as file:
        ...
        for gen in generation_numbers:
            ...
            for car in tracked_cars.values():
                if car["all_zones_visited_generation"] == gen:
...
def update_tracked_cars(cars, generation):
    ...
    for i, car in enumerate(cars[:50]):
        ...
        visited_zones_in_gen = set(zone for zone, _ in current_gen_visits)
        if len(visited_zones_in_gen) == 3 and tracked_cars[i]["all_zones_visited_generation"] is None:
    ...
    with open(filename, "w") as file:
        for i in range(50):
            ...
            if tracked_cars[i]["all_zones_visited_generation"] is not None:
                ...
                file.write(f"All 3 zones first visited in Generation: {all_zones_gen}\n")
                ...
                if all_zones_gen in tracked_cars[i]['visit_sequences']:
                    ...
                    print(f"Warning: Missing data for all 3 zones in Generation {all_zones_gen} for Car {i}")
                    file.write("Data for all zones in the specified generation is missing.\n")
                    tracked_cars[i]["all_zones_visited_generation"] = None
            file.write("\n")
