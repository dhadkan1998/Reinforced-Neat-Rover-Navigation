...
def update_tracked_cars(cars, generation):
    ...
    for i, car in enumerate(cars[:50]):  # Only consider the first 50 cars
        ...
        if new_zone_discovered:
            ...
            print(f"Updated visit sequence for Car {i} in Generation {generation}: {current_gen_visits}")  # Debug print statement

        # Check if all 4 zones were visited this generation
        visited_zones_in_gen = set(zone for zone, _ in current_gen_visits)
        if len(visited_zones_in_gen) == 4 and tracked_cars[i]["all_zones_visited_generation"] is None:
            tracked_cars[i]["all_zones_visited_generation"] = generation
            print(f"Car {i} visited all 4 zones in Generation {generation}")  # Debug print statement
    ...
    with open(filename, "w") as file:
        for i in range(50):
            ...
            if tracked_cars[i]["all_zones_visited_generation"] is not None:
                ...
                file.write(f"All 4 zones first visited in Generation: {all_zones_gen}\n")
                file.write("Times for each zone in that generation:\n")
                
                # Check if we have data for the generation where all zones were first visited
                if all_zones_gen in tracked_cars[i]['visit_sequences']:
                    ...
                    print(f"Warning: Missing data for all 4 zones in Generation {all_zones_gen} for Car {i}")
                    file.write("Data for all zones in the specified generation is missing.\n")
                    tracked_cars[i]["all_zones_visited_generation"] = None  # Reset this value
