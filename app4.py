    ...
    with open(filename, "w") as file:
        for i in range(50):
            ...
            if tracked_cars[i]["all_zones_visited_generations"]:
                file.write(f"Generations where all 4 zones were visited: {sorted(tracked_cars[i]['all_zones_visited_generations'])}\n")
                ...
                if first_all_zones_gen in tracked_cars[i]['visit_sequences']:
                    ...
                    print(f"Warning: Missing data for all 4 zones in Generation {first_all_zones_gen} for Car {i}")
